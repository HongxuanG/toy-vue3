...
这里关于template怎么编译成具有render函数的特殊对象，我们暂时忽略。它涉及AST的知识。
...
创建vue上下文的时候，createApp接受根组件选项对象或者根组件组合对象，并返回一系列的应用API（component、mount、config、use等等）

通过上述的功能描述，我们可以简单实现以下createApp
```typescript
...
```
按照官方文档来说`h()`函数的实现也是`createVNode()`，返回一个虚拟节点。
```typescript
