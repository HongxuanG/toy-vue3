...
但是**h函数**的第三个参数不是一个vnode节点了，而是一个数组，这时候`this.$slots`就是一个数组了，这相当于子组件return的`h('div', {}, [foo, this.$slots])`，到了mountChildren这一步 `this.$slots` 并不能patch。因为`this.$slots`并不是 `vnode`。
...
export default {
  ...
  render() {
    ...
    const foo = h('p', {}, '原本就在Foo里面的元素')
    // 结果就是'aaa'没有渲染出来
    return h('div', {}, [foo, renderSlot(this.$slots, 'default'), 'aaa'])
  }
}
```
